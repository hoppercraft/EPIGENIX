...
import { IconArrowRight, IconCheckCircle, IconFile } from '../components/Icons'
...
function WorkspaceDashboard() {
	const { geneticData, setGeneticData } = useGeneticData()
	...
	const displayedDiseaseSlug =
		uploadState.diseaseSlug ||
		detectDisease(extractDiseaseFromObject(displayedDataObject))

	const displayedFileName = uploadState.fileName || 'Stored patient data'
		...
		reader.onload = () => {
			try {
				...
				try {
					...
					const firstItem = Array.isArray(parsedData)
						? parsedData[0]
						: parsedData
					...
					{uploadState.status === 'error' ? (
						...
						<div className="mt-3 rounded-2xl border border-outline-variant/60 bg-surface-lowest/60 p-4">
							...
							<p className="mt-2 text-xs text-muted">
								Source file: {displayedFileName}
							</p>
						</div>
					) : displayedData ? (
						<p className="mt-3 text-xs text-muted">
							Stored patient data found. Upload a new file to replace it.
						</p>
					) : (
						<p className="mt-3 text-xs text-muted">
					...
					<section className="rounded-3xl border border-outline-variant/60 bg-surface-lowest/50 p-6 shadow-glass backdrop-blur-glass">
						<div className="flex flex-wrap items-start justify-between gap-4">
							<div>
								...
								<h3 className="mt-2 font-display text-xl font-semibold tracking-tight text-ink">
									{displayedDataObject?.patient_id ||
										displayedDataObject?.patientId ||
										'Patient data'}
								</h3>
							</div>
						...
						<div className="mt-6 grid gap-4 sm:grid-cols-3">
							...
							<article className="rounded-2xl border border-outline-variant/60 bg-surface-lowest/60 p-4">
								...
								<p className="mt-2 text-sm font-semibold text-ink">
									{displayedDataObject?.age || 'Not provided'}
								</p>
							</article>
							...
							<article className="rounded-2xl border border-outline-variant/60 bg-surface-lowest/60 p-4">
								...
								<p className="mt-2 text-sm font-semibold text-ink">
									{displayedDiseaseSlug === 'unknown'
										? 'Not provided'
										: displayedDiseaseSlug}
								</p>
							</article>
						</div>
	...
	const selectedDiseaseSlug = displayedDiseaseSlug
	const isUnknown = selectedDiseaseSlug === 'unknown'

	const fallbackDisease =
		diseaseCatalog[diseaseOptions[0]?.slug] || diseaseOptions[0]

	const activeDisease =
		diseaseCatalog[selectedDiseaseSlug] || fallbackDisease
					...
					<div className="glass-card-sm p-5">
						...
						<div className="space-y-4">
							{topDrivers.map((driver, i) => (
								<div
									key={driver.id}
									className={`space-y-2 anim-fade-up anim-delay-${i + 1}`}
								>
									<div className="flex items-center justify-between text-sm">
										<p className="font-medium text-[rgb(var(--on-surface))]">
											{driver.label}
										</p>

										<span
											className={`badge-purple !py-0.5 !text-[9px] ${
												driver.type === 'Gene' ? '' : 'badge'
											}`}
										>
											{driver.type}
										</span>
									...
									<div className="h-1.5 w-full overflow-hidden rounded-full bg-[rgba(var(--outline-variant),0.2)]">
										<div
											...
											style={{
												...
												background:
													'linear-gradient(90deg, rgb(var(--primary)), rgb(var(--accent-purple)))',
