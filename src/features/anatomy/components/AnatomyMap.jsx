...
function AnatomyMap({ diseaseConfig, markerData, onMarkerHover, onMarkerBlur, hoveredMarker }) {
	const titleId = useId()
	const descId = useId()

	const regions = diseaseConfig.affectedRegions || []
	let minX = 50, maxX = 50, minY = 80, maxY = 80
	if (regions.length > 0) {
		minX = Math.min(...regions.map((r) => r.cx - r.rx))
		maxX = Math.max(...regions.map((r) => r.cx + r.rx))
		minY = Math.min(...regions.map((r) => r.cy - r.ry))
		maxY = Math.max(...regions.map((r) => r.cy + r.ry))
	} else if (markerData && markerData.length > 0) {
		minX = Math.min(...markerData.map((m) => m.x))
		maxX = Math.max(...markerData.map((m) => m.x))
		minY = Math.min(...markerData.map((m) => m.y))
		maxY = Math.max(...markerData.map((m) => m.y))
	}
	...
	return (
		<svg
			...
			className="anatomy-svg w-full max-w-sm mx-auto"
		>
			...
			<defs>
				<linearGradient id="liquidGlass" x1="0%" y1="0%" x2="100%" y2="100%">
					...
					<stop offset="30%" stopColor="rgba(45, 212, 191, 0.15)" />
			...
			<g
				style={{
					...
					transform: `translate(${50 - centerX}px, ${80 - centerY}px) scale(${zoomScale})`,
					transition: 'all 0.8s cubic-bezier(0.4, 0, 0.2, 1)',
				}}
			>
				<g className="anatomy-body" transform="scale(0.484668)" style={{ pointerEvents: 'none' }}>
					<path
				...
				<g className="anatomy-regions" style={{ pointerEvents: 'none' }}>
				...
				<g className="anatomy-markers">
					{markerData.map((marker) => {
						const isHovered = hoveredMarker?.rsId === marker.rsId
						return (
						<g key={marker.rsId}>
							{/* Visible Circle */}
							<circle
								cx={marker.x}
								cy={marker.y}
								r={isHovered ? marker.radius * 1.5 : marker.radius}
								opacity={marker.opacity}
								className="risk-marker"
								filter={isHovered ? "drop-shadow(0 0 8px rgba(239, 68, 68, 0.8))" : "url(#riskGlow)"}
								style={{ pointerEvents: 'none', transition: 'r 0.15s ease, filter 0.15s ease' }}
							/>
							{/* Invisible Hit Area */}
							<circle
								cx={marker.x}
								cy={marker.y}
								r={Math.max(marker.radius * 2.5, 4)}
								fill="transparent"
